...
public class IntArrayIterator implements Iterator<IntArrayIterator.Item> {
    ...
    public IntArrayIterator(int @NotNull ... array) {
        this(array, 0, array.length);
    }

    public IntArrayIterator(int @NotNull [] array, int pos, int length) {
        this.array = array;
        this.idx   = pos;
     ...
     */
    public @Override IntArrayIterator.Item next() {
        if(!hasNext()) throw new NoSuchElementException();
        int i = idx++;
        return new Item(i, array[i]);
    }

    public record Item(int idx, int value) implements Comparable<Item> {
