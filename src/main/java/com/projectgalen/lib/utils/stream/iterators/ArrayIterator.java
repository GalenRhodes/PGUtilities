...
public class ArrayIterator<T> implements Iterator<ArrayIterator.Item<T>> {
    ...
    public ArrayIterator(T @NotNull [] array, int pos, int length) {
        this.array = array;
        this.idx   = pos;
     ...
     */
    public @Override ArrayIterator.Item<T> next() {
        if(!hasNext()) throw new NoSuchElementException();
        int i = idx++;
        return new Item<>(i, array[i]);
    }

    public record Item<T>(int idx, T value) implements Comparable<Item<T>> {
        public @Override int compareTo(@NotNull Item<T> o) { return Integer.compare(idx, o.idx); }
    }
}
