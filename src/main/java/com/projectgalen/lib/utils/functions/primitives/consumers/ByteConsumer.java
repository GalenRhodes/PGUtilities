...
/**
 * Represents an operation that accepts a single {@code byte}-valued argument and returns no result.  This is the primitive type specialization of {@link java.util.function.Consumer} for {@code byte}.
...
public interface ByteConsumer {
    ...
    /**
     * Returns a composed {@code ByteConsumer} that performs, in sequence, this operation followed by the {@code after} operation. If performing either operation throws an exception, it is relayed to
     ...
     */
    default @NotNull ByteConsumer andThen(@NotNull ByteConsumer after) {
        return (byte t) -> {
            accept(t);
            after.accept(t);
        };
